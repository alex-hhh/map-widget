...
with the mouse, as well as programatically.  There is a tutorial on how to use
this widget, it is available
...
@defconstructor[([parent (or/c (is-a?/c frame%) (is-a?/c dialog%) (is-a?/c
panel%) (is-a?/c pane%))] [position (or/c #f (vector/c flonum? flonum?))])]{
...
When the track group is @racket[#f], the default pen is updated, this is used
for tracks that have no group defined (i.e. the group is @racket[#f])}
...
will be drawn.  Tracks with a smaller Z-order will be drawn before tracks with
a larger Z-order. The draw order for tracks with the same Z-order value is
unspecified.}
...
@defmethod[(move-to (position (vector/c flonum? flonum?))) any/c]{Center the
map around the specified @racket[position], a vector containing the latitude
and longitude}
...
@defproc[(get-tile-provider-names) (listof string?)]{Returns a list of
available tile providers.

By default only the Open Street Map tile server is available.  To use
@hyperlink["http://thunderforest.com/"]{Thunderforest} tiles, you will need to
obtain an API key and set the @tt{AL2TFAPIKEY} environment variable with that
API key.}

@defproc[(current-tile-provider-name) string?]{Return the name of the
current tile provider}

@defproc[(set-current-tile-provider [name string?]) any/c]{Set the name of the
tile provider to use.  All instances of @racket[map-widget%] will use the same
tile provider.
