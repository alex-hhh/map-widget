...
(define map-widget%/c
  (class/c
   ...
   (set-current-location (->m (or/c (vector/c flonum? flonum?) #f) any/c))
   (set-track-current-location (->m boolean? any/c))
      ...
      (when current-location
        (let-values (([w h] (send canvas get-size)))
          (let* ((point (lat-lon->npoint
                         (point-lat current-location) (point-lon current-location)))
